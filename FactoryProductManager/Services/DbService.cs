...
namespace FactoryProductManager.Services
{
    public class DbService
    {
        ...
        private void EnsureFactoryProductsSchema(SQLiteConnection conn)
        {
            ...
            if (!ColumnExists(conn, "FactoryProducts", "supply_cycle"))
            {
                using var cmd = new SQLiteCommand("ALTER TABLE FactoryProducts ADD COLUMN supply_cycle TEXT", conn);
                cmd.ExecuteNonQuery();
            }
        }
        ...
        private void EnsureProductPartMaterialsSchema(SQLiteConnection conn)
        {
            ...
            // 图纸编号列
            if (!ColumnExists(conn, "ProductPartMaterials", "drawing_number"))
            {
                using var cmd = new SQLiteCommand("ALTER TABLE ProductPartMaterials ADD COLUMN drawing_number TEXT", conn);
                cmd.ExecuteNonQuery();
                LogService.Info("[ProductPartMaterials] 迁移：添加 drawing_number 列");
            }
        }
        ...
        public List<MaterialGroup> GetMaterialGroups()
        {
            ...
            try
            {
                ...
                if (list.Count > 0)
                {
                    ...
                    {
                        int gid = Convert.ToInt32(itemReader.GetValue(1));
        ...
        /// <summary>
        /// 种子数据：统一为 1 个柜类模板。
        /// GX-001：柜类通用（柜体+门板+台面(可选项)+五金）
        /// 强制删除旧的 CB-001/XYG-001/ZDT-001 并重新插入。
        /// </summary>
        public void SeedDefaultMaterialGroups()
        {
            try
            {
                ...
                foreach (var code in oldCodes)
                {
                    ...
                    delItemCmd.Parameters.AddWithValue("@code", code);
                    delItemCmd.ExecuteNonQuery();

                    var delGroupCmd = new SQLiteCommand("DELETE FROM MaterialGroups WHERE group_code = @code", conn);
                    delGroupCmd.Parameters.AddWithValue("@code", code);
            ...
            try
            {
                using (var conn = GetConnection())
                {
                    ...
                    INSERT INTO FactoryProducts (factory_product_code, my_product_code, product_name,
                        ...
                        supply_cycle, category, image_url, factory_id)
                    VALUES (@factoryCode, @myCode, @name, @brand, @spec, @texture, @process, @unit, @costPrice,
                        @scenario, @cert, @supplyCycle, @category, @image, @factoryId);
                    SELECT last_insert_rowid();", conn);
                    ...
                    cmd.Parameters.AddWithValue("@supplyCycle", ToDbValue(material.SupplyCycle));
        ...
        private void InsertProductPartsInternal(SQLiteConnection conn, SQLiteTransaction tx, int productId, IReadOnlyList<ProductPart> parts)
        {
            ...
            foreach (var p in parts)
            {
                ...
                LogService.Info($"新增产品部件成功: ID={newId}, 名称={p.PartName}");
            }
        }
        ...
        public List<ProductPart> GetProductParts(int productId)
        {
            ...
            try
            {
                ...
                LogService.Info($"查询产品部件列表完成，共 {parts.Count} 条记录");
                return parts;
            }
            catch (Exception ex)
            {
                LogService.Error($"查询产品部件列表失败: productId={productId}", ex);
                throw;
            }
        }
        ...
        public int AddProductPart(ProductPart part)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    ...
                    LogService.Info($"新增产品部件成功: ID={partId}, 名称={part.PartName}");
                    return partId;
                }
            }
            catch (Exception ex)
            {
                LogService.Error($"新增产品部件失败: 名称={part.PartName}", ex);
                throw;
            }
        }
        ...
        public void UpdateProductPart(ProductPart part)
        {
            try
            {
                ...
                LogService.Info($"更新产品部件成功: ID={part.Id}, 名称={part.PartName}");
            }
            catch (Exception ex)
            {
                LogService.Error($"更新产品部件失败: ID={part.Id}", ex);
                throw;
            }
        }
        ...
        public void DeleteProductPart(int id)
        {
            try
            {
                ...
                LogService.Info($"删除产品部件成功: ID={id}");
            }
            catch (Exception ex)
            {
                LogService.Error($"删除产品部件失败: ID={id}", ex);
                throw;
            }
        }
            ...
            try
            {
                using (var conn = GetConnection())
                {
                    ...
                    using (var tx = conn.BeginTransaction())
                    {
                        var cmd = new SQLiteCommand(@"
                            INSERT INTO ProductPartMaterials
                                (product_id, part_id, part_name, component_name, material_type_name,
                                 ...
                                 remarks, is_composite, group_code, item_name, parent_id, drawing_number,
                            ...
                            VALUES
                                (@productId, @partId, @partName, @componentName, @materialTypeName,
                                 ...
                                 @remarks, @isComposite, @groupCode, @itemName, @parentId, @drawingNumber,
                                 @createdAt, @updatedAt);
                            SELECT last_insert_rowid();", conn, tx);
                        ...
                        cmd.Parameters.Add("@drawingNumber", System.Data.DbType.String);
                        ...
                        foreach (var m in materials)
                        {
                            ...
                            cmd.Parameters["@drawingNumber"].Value = ToDbValue(m.DrawingNumber);
                        ...
                        tx.Commit();
                        LogService.Info($"新增产品部件物料成功: productId={productId}, 共 {count} 条");
                        return count;
                    }
                }
            }
            ...
            try
            {
                ...
                var cmd = new SQLiteCommand(@"
                    ...
                    SET part_id=@partId, part_name=@partName, component_name=@componentName, material_type_name=@materialTypeName,
                        ...
                        drawing_number=@drawingNumber, updated_at=@updatedAt
                    WHERE id=@id", conn);
                ...
                cmd.Parameters.AddWithValue("@drawingNumber", ToDbValue(m.DrawingNumber));
        ...
        public List<ProductPartMaterial> GetProductPartMaterials(int productId, int? partId = null)
        {
            ...
            try
            {
                using (var conn = GetConnection())
                {
                    conn.Open();
                    var cmd = new SQLiteCommand(@"
                        SELECT ppm.id, ppm.product_id, ppm.part_id, ppm.part_name, ppm.component_name, ppm.material_type_name,
                               ppm.material_id, ppm.material_name, ppm.factory_material_code, ppm.my_material_code,
                               ppm.brand, ppm.specification, ppm.unit, ppm.unit_price, ppm.quantity, ppm.total_price,
                               ppm.remarks, ppm.is_composite, ppm.group_code, ppm.item_name, ppm.parent_id, ppm.drawing_number,
                        ...
                        LEFT JOIN Factories f ON fp.factory_id = f.id
                        WHERE ppm.product_id = @productId" + (partId.HasValue ? " AND ppm.part_id = @partId" : "") + @"
                        ORDER BY ppm.part_name, ppm.component_name, ppm.id", conn);
                    cmd.Parameters.AddWithValue("@productId", productId);
                    if (partId.HasValue)
                    {
                    ...
                    while (reader.Read())
                    {
                        list.Add(new ProductPartMaterial
                        {
                            ...
                            DrawingNumber = reader.IsDBNull(21) ? string.Empty : Convert.ToString(reader.GetValue(21)),
                            FactoryName = reader.IsDBNull(24) ? string.Empty : Convert.ToString(reader.GetValue(24)),
                            ImageUrl = reader.IsDBNull(25) ? string.Empty : Convert.ToString(reader.GetValue(25)),
                            CreatedAt = reader.IsDBNull(22) ? DateTime.Now : DateTime.Parse(Convert.ToString(reader.GetValue(22))),
                            UpdatedAt = reader.IsDBNull(23) ? DateTime.Now : DateTime.Parse(Convert.ToString(reader.GetValue(23)))
                        });
                    }
                }
                LogService.Info($"查询产品部件物料完成: productId={productId}, partId={partId}, 共 {list.Count} 条");
                return list;
            }
            catch (Exception ex)
            {
                LogService.Error($"查询产品部件物料失败: productId={productId}", ex);
                throw;
            }
        }
        ...
        public int DeleteProductPartMaterialsByProduct(int productId)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    ...
                    LogService.Info($"清空产品部件物料: productId={productId}, 删除 {n} 条");
                    return n;
                }
            }
            catch (Exception ex)
            {
                LogService.Error($"清空产品部件物料失败: productId={productId}", ex);
                throw;
            }
        }
        ...
        public List<CustomPart> GetCustomParts()
        {
            ...
            try
            {
                ...
                LogService.Info($"查询自定义部件列表完成，共 {list.Count} 条记录");
                return list;
            }
            catch (Exception ex)
            {
                LogService.Error("查询自定义部件列表失败", ex);
                throw;
            }
        }
        ...
        public int AddCustomPart(CustomPart part)
        {
            ...
            try
            {
                using (var conn = GetConnection())
                {
                    ...
                    {
                        throw new InvalidOperationException($"自定义部件\"{part.PartName}\"已存在");
                    }
                    ...
                    int newId = Convert.ToInt32(cmd.ExecuteScalar());
                    part.Id = newId;
                    LogService.Info($"新增自定义部件成功: ID={newId}, 名称={part.PartName}");
                    return newId;
                }
            }
            catch (Exception ex)
            {
                LogService.Error($"新增自定义部件失败: 名称={part.PartName}", ex);
                throw;
            }
        }
        ...
        public void UpdateCustomPart(CustomPart part)
        {
            ...
            {
                throw new ArgumentException("自定义部件 ID 无效");
            }
            ...
            try
            {
                ...
                LogService.Info($"更新自定义部件成功: ID={part.Id}, 名称={part.PartName}");
            }
            catch (Exception ex)
            {
                LogService.Error($"更新自定义部件失败: ID={part.Id}", ex);
                throw;
            }
        }
        ...
        public void DeleteCustomPart(int id)
        {
            try
            {
                ...
                LogService.Info($"删除自定义部件成功: ID={id}");
            }
            catch (Exception ex)
            {
                LogService.Error($"删除自定义部件失败: ID={id}", ex);
                throw;
            }
        }
        ...
        public bool CustomPartExists(string partName, int? excludeId = null)
        {
            ...
            {
                LogService.Error($"检查自定义部件是否存在失败: name={partName}", ex);
                return false;
            }
        }
        ...
        public List<string> GetAllProjectCodes()
        {
            try
            {
                using var conn = GetConnection();
                conn.Open();
                var sql = "SELECT DISTINCT project_name FROM Products WHERE project_name IS NOT NULL AND project_name <> '' ORDER BY project_name";
                var cmd = new SQLiteCommand(sql, conn);
                using var reader = cmd.ExecuteReader();
        ...
        /// <summary>
        /// 检查项目内图纸编号是否已存在（同时检查普通物料和复合物料）
        /// </summary>
        /// <returns>(exists, source, productCode, partName, componentName)</returns>
        public (bool exists, string source, string productCode, string partName, string componentName)? CheckDrawingNumberExistsInProject(string projectCode, string drawingNumber, int? excludeProductId = null)
        {
            try
            {
                using var conn = GetConnection();
                ...
                // 1. 检查 ProductMaterialLibrary（普通物料）
                var pmlSql = @"
                    SELECT p.product_code, pml.part_name, pml.component_name
                    FROM ProductMaterialLibrary pml
                    INNER JOIN Products p ON pml.product_id = p.id
                    WHERE p.product_code LIKE @projectCode || '%'
                      AND pml.drawing_number = @drawingNumber";
                if (excludeProductId.HasValue)
                {
                    pmlSql += " AND pml.product_id <> @excludeId";
                }
                pmlSql += " LIMIT 1";

                using (var cmd = new SQLiteCommand(pmlSql, conn))
                {
                    cmd.Parameters.AddWithValue("@projectCode", projectCode);
                    ...
                    if (reader.Read())
                    {
                        var productCode = reader.IsDBNull(0) ? "" : reader.GetString(0);
                        var partName = reader.IsDBNull(1) ? "" : reader.GetString(1);
                        var componentName = reader.IsDBNull(2) ? "" : reader.GetString(2);
                        return (true, "ProductMaterialLibrary", productCode, partName, componentName);
                    }
                }

                // 2. 检查 ProductPartMaterials（复合物料主行）
                var ppmSql = @"
                    SELECT p.product_code, ppm.part_name, ppm.component_name
                    FROM ProductPartMaterials ppm
                    INNER JOIN Products p ON ppm.product_id = p.id
                    WHERE p.product_code LIKE @projectCode || '%'
                      AND ppm.drawing_number = @drawingNumber
                      AND ppm.is_composite = 1";  // 只检查复合物料主行
                if (excludeProductId.HasValue)
                {
                    ppmSql += " AND ppm.product_id <> @excludeId";
                }
                ppmSql += " LIMIT 1";

                using (var cmd = new SQLiteCommand(ppmSql, conn))
                {
                    cmd.Parameters.AddWithValue("@projectCode", projectCode);
                    cmd.Parameters.AddWithValue("@drawingNumber", drawingNumber);
                    if (excludeProductId.HasValue)
                    ...
                    if (reader.Read())
                    {
                        ...
                        var partName = reader.IsDBNull(1) ? "" : reader.GetString(1);
                        var componentName = reader.IsDBNull(2) ? "" : reader.GetString(2);
                        return (true, "ProductPartMaterials", productCode, partName, componentName);
                    }
                }
